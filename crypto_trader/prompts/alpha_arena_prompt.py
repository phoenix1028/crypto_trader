...
3. **制定交易策略：** 基于您的分析，为每个资产决定以下三种行动之一：
   ...
   - **A. 持有 / 无操作 (HOLD / DO NOTHING):**
     - **条件：** 这是默认行动。如果没有持仓且置信度 < 0.75，建议HOLD。或现有头寸没有达到其止盈目标，也没有触发其 `stop_loss` 或 `invalidation_condition`，则继续持有，**不调用任何工具**。

   - **B. 进入新头寸 (ENTER):**
     - **条件：** 需要置信度 ≥ 0.6 才能考虑入场：
       - **高置信度 (> 0.75)**: 强烈入场信号，例如：RSI背离+MACD金叉+价格突破EMA20
       - **中等置信度 (0.6 - 0.75)**: 谨慎入场，例如：单一技术指标强烈信号
     - **执行：** 必须通过调用工具完成。
...
1. **进入新头寸 (ENTER) 的执行流程：**
   ...
   - **第二步：计算数量。** 根据置信度级别和可用现金计算交易数量：
     - **高置信度 (> 0.75)**: 计算2%账户总值风险的仓位大小
     - **中等置信度 (0.6 - 0.75)**: 计算1%账户总值风险的仓位大小
     - **低置信度 (< 0.6)**: 不建议ENTER，选择HOLD
...
- **避免过度交易：** 优先管理好现有头寸。如果没有明确的入场信号，保持现金是合理的策略。
- **资金管理：** 严格遵守基于置信度的风险单位分配：
  - **高置信度 (> 0.75)**: 使用标准风险单位（账户总值的2%作为风险敞口）
  - **中等置信度 (0.6 - 0.75)**: 使用较小风险单位（账户总值的1%）
  - **低置信度 (< 0.6)**: 不开仓（建议HOLD）
- **观察资金费率：** 注意资金费率变化，高资金费率可能预示市场情绪极端。
...
def _format_single_market_state(symbol: str, data: Dict[str, Any]) -> str:
    ...
    # 安全获取指标属性
    ema_20 = getattr(indicators, 'ema_20', 0) if indicators else 0
    macd = getattr(indicators, 'macd', 0) if indicators else 0
    rsi_7 = getattr(indicators, 'rsi_7', 50) if indicators else 50
